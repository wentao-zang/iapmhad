package com.iapmhad.hdymzkjqrxt.controller;

import java.util.Arrays;
import java.util.Map;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import com.iapmhad.hdymzkjqrxt.entity.XtLogEntity;
import com.iapmhad.hdymzkjqrxt.service.XtLogService;
import com.iapmhad.common.utils.PageUtils;
import com.iapmhad.common.utils.R;
import org.springframework.cloud.context.config.annotation.RefreshScope;




/**
 * 
 *
 * @author ynd
 * @email 3361725156@qq.com
 * @date 2022-07-04 09:04:25
 */
@RefreshScope
@RestController
@RequestMapping("hdymzkjqrxt/xtlog")
public class XtLogController {
    @Autowired
    private XtLogService xtLogService;

    /**
     * 列表
     */
    @RequestMapping("/list")
    ////@RequiresPermissions("hdymzkjqrxt:xtlog:list")
    public R list(@RequestParam Map<String, Object> params){
        PageUtils page = xtLogService.queryPage(params);

        return R.ok().put("page", page);
    }

    /*
     *获取大于{id}的所有数据
     *
     * */
    @RequestMapping("/getLi/{id}")
    public List<XtLogEntity> getLi(@PathVariable("id") Integer id) {
        List<XtLogEntity> list = xtLogService.getLi(id);
        if(list.size()>100) {
            list = list.subList(list.size() - 100,list.size());
        }
        return list;
    }
    /*
     *获取最后一个的数据
     *
     * */
    @RequestMapping("/getLast")
    public XtLogEntity getLast(){
        XtLogEntity XtLog = xtLogService.getLast();
        return  XtLog;
    }
    /**
     * 信息
     */
    @RequestMapping("/info/{id}")
    ////@RequiresPermissions("hdymzkjqrxt:xtlog:info")
    public R info(@PathVariable("id") Integer id){
		XtLogEntity xtLog = xtLogService.getById(id);

        return R.ok().put("xtLog", xtLog);
    }

    /**
     * 保存
     */
    @RequestMapping("/save")
    ////@RequiresPermissions("hdymzkjqrxt:xtlog:save")
    public R save(@RequestBody XtLogEntity xtLog){
		xtLogService.save(xtLog);

        return R.ok();
    }

    /**
     * 修改
     */
    @RequestMapping("/update")
    ////@RequiresPermissions("hdymzkjqrxt:xtlog:update")
    public R update(@RequestBody XtLogEntity xtLog){
		xtLogService.updateById(xtLog);

        return R.ok();
    }

    /**
     * 删除
     */
    @RequestMapping("/delete")
    ////@RequiresPermissions("hdymzkjqrxt:xtlog:delete")
    public R delete(@RequestBody Integer[] ids){
		xtLogService.removeByIds(Arrays.asList(ids));

        return R.ok();
    }

}
