    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/processParametersJqrzk/noncoldextrusionholetool/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.toolName = data.nonColdExtrusionHoleTool.toolName
                this.dataForm.toolAperture = data.nonColdExtrusionHoleTool.toolAperture
                this.dataForm.edgeLength = data.nonColdExtrusionHoleTool.edgeLength
                this.dataForm.toolLife = data.nonColdExtrusionHoleTool.toolLife
                this.dataForm.noseLength = data.nonColdExtrusionHoleTool.noseLength
                this.dataForm.lengthmeter = data.nonColdExtrusionHoleTool.lengthmeter
                this.dataForm.storehosePos = data.nonColdExtrusionHoleTool.storehosePos
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/processParametersJqrzk/noncoldextrusionholetool/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
